...
const AfiseazaReteteTreptatIstoric = ({ DataToPrint }: any) => {
    const { Retete, DeleteMode, changeFormularANDRetete, ComponentFormularANDRetete, FolderPath } = DataToPrint

    const [nrReteteDeAfisat, changeNrAfisare] = useState<number>(0)


    const DeleteSpecifiedReteta = (Retetapath: string) => {
        const NewComponentFormularANDRetete = ComponentFormularANDRetete.map((component: any) => {
            if (component.FolderPath !== FolderPath)
                return component
            return { ...component, Retete: Retete.filter((Reteta: any) => { return Reteta.path !== Retetapath }) }
        })
